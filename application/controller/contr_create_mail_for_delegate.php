<?php
   ...
   //     $today_date = $list_mail[0]['DATE_START'];
        ...
        {
            $sql_upd_state = "update DOC_RECIEPMENTS set COMMENT_TO_DOC = '$comment', STATE = '5', POST_DATE = '$today_date', POST_TIME= '$now_time' where ID = '$REC_ID'";
            $list_upd_state = $db -> Select($sql_upd_state);

            foreach ($_POST['RECIPIENT'] as $key=>$reciep_mail)
            {
                $sql_reciep = "INSERT INTO DOC_RECIEPMENTS (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, DESTINATION, COMMENT_TO_DOC, RECIEP_DATE, RECIEP_TIME) VALUES (SEQ_DOC_RECIEPMENTS.NEXTVAL, '$mail_id', '$reciep_mail', '0', '$emp_mail', '$DESTINATION', '', '$today_date', '$now_time')";
                $list_reciep = $db -> Select($sql_reciep);
                $document->sendmail($reciep_mail, 'Уведомление в СЭД', "Сотрудник $emp_fio прислал письмо на исполнение с комментарием $comment. Для ознакомления пройдите по ссылке http://192.168.5.244/mail_detail?doc_id=$mail_id&get_file=$mail_id&rec_id=$REC_ID&dest_id=$DESTINATION&step_id=81&next_step_id=84&state_id=4&prev_step_id=0&table_name_eng=DOC_RECIEPMENTS");
              //  mail("$reciep_mail", 'Уведомление в СЭД', "Сотрудник $emp_fio прислал письмо на исполнение с комментарием '$comment'. Для ознакомления пройдите по ссылке http://192.168.5.244/mail_detail?doc_id=$mail_id&get_file=$mail_id&rec_id=$REC_ID&dest_id=$DESTINATION&step_id=81&next_step_id=84&state_id=4&prev_step_id=0&table_name_eng=DOC_RECIEPMENTS", "From: Система Электронного Документооборота");
            }

            foreach ($_POST['RECIEPMENTS_CONTROL'] as $key=>$reciep_mail)
            {
                $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_CONTROL (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, RECIEP_DATE, RECIEP_TIME) VALUES (SEQ_DOC_RECIEPMENTS_CONTROL.NEXTVAL, '$mail_id', '$reciep_mail', '0', '$emp_mail', '$today_date', '$now_time')";
                $list_reciep = $db -> Select($sql_reciep);
                if($reciep_mail == 'g.amerkhojayev@gak.kz') $reciep_mail = 'z.kassenova@gak.kz';
                $document->sendmail($reciep_mail, 'Уведомление в СЭД', "Сотрудник $emp_fio отправил Вам на контроль с коротким описанием $short_text, добавив комментарий $comment. Для ознакомления пройдите по ссылке http://192.168.5.244/on_my_control.");
               // mail("$reciep_mail", 'Уведомление в СЭД', "Сотрудник $emp_fio отправил Вам на контроль с коротким описанием '$short_text', добавив комментарий '$comment'. Для ознакомления пройдите по ссылке http://192.168.5.244/on_my_control.", "From: Система электронного документооборота");
            }
