<?php
    ...
    $sql_inbox_num = "select * from DIC_INBOX_NUM where id = 1";
    $list_inbox_num = $db->select($sql_inbox_num);
    $inbox_num = $list_inbox_num[0]['INBOX_NUM'];   
    ...
    {        
        ...
        $SENDER_REG_NUM = $_POST['SENDER_REG_NUM'];
        $SENDER_KIND = $_POST['SENDER_KIND'];
        $TRIP = $_POST['TRIP'];
        ...
        {            
            ...
            {
                $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_AGREEMENT (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, RECIEP_DATE, RECIEP_TIME, DESTINATION, AGREEMENT_TYPE) VALUES (SEQ_DOC_RECIEPMENTS_AGREEMENT.NEXTVAL, '$id', '$reciep_mail', '1', '$emp_mail', '$today_date', '$now_time', '$DESTINATION', '1')";
                $list_reciep = $db -> Select($sql_reciep);
    
                if($reciep_mail == 'g.amerkhojayev@gak.kz') $reciep_mail = 'z.kassenova@gak.kz';
                $document->sendmail($reciep_mail, 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на согласование, с коротким описанием $SHORT_TEXT. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new");
            //  mail("$reciep_mail", 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на согласование, с коротким описанием '$SHORT_TEXT'. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new", "From: Система электронного документооборота");
            }
            ...
            foreach ($_POST['RECIPIENT_REGISTRATION'] as $key=>$reciep_mail)
            {
                ...
                if($reciep_mail == 'g.amerkhojayev@gak.kz') $reciep_mail = 'z.kassenova@gak.kz';
                $document->sendmail($reciep_mail, 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на регистрацию, с коротким описанием $SHORT_TEXT. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new");
             //   mail("$reciep_mail", 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на регистрацию, с коротким описанием '$SHORT_TEXT'. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new", "From: Система электронного документооборота");
            }
            ...
            foreach ($_POST['RECIPIENT_RESOLUTION'] as $key=>$reciep_mail)
            {
                ...
                if($reciep_mail == 'g.amerkhojayev@gak.kz') $reciep_mail = 'z.kassenova@gak.kz';
                $document->sendmail($reciep_mail, 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на резолюцию, с коротким описанием $SHORT_TEXT. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new");
             //   mail("$reciep_mail", 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на резолюцию, с коротким описанием '$SHORT_TEXT'. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new", "From: Система электронного документооборота");
            }
            ...
            foreach ($_POST['RECIPIENT'] as $key=>$reciep_mail)
            {
                ...
                if($reciep_mail == 'g.amerkhojayev@gak.kz') $reciep_mail = 'z.kassenova@gak.kz';
                $document->sendmail($reciep_mail, 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на исполнение, с коротким описанием $SHORT_TEXT. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new");
            //    mail("$reciep_mail", 'Новое письмо в СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на исполнение, с коротким описанием '$SHORT_TEXT'. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new", "From: Система электронного документооборота");
            }
        ...
        $sql_create_mail = "INSERT INTO DOCUMENTS (ID, TYPE, STATE, REG_NUM, HEAD_TEXT, SENDER, SENDER_MAIL, DATE_START, DATE_END, LINK_FROM, SHORT_TEXT, KIND, DOC_LINK, CURRENT_STEP_ID, NEXT_STEP_ID, PREV_STEP_ID, ORG_SENDER, SENDER_REG_NUM, SENDER_KIND) VALUES ($id, '$type', '$state', '$REG_NUM', '$HEAD_TEXT', '$SENDER', '$SENDER_MAIL', '$DATE_START', '$DATE_END', '$LINK_FROM', '$SHORT_TEXT', '$KIND', '$DOC_LINK', '$CURRENT_STEP_ID', '$NEXT_STEP_ID', '$PREV_STEP_ID', '$ORG_SENDER', '$SENDER_REG_NUM', '$SENDER_KIND')";
        ...
        $sql_update_last_JB_num = "update DIC_INBOX_NUM set INBOX_NUM =  INBOX_NUM+1 where ID = 1";
    ...
    $sql_reg_num_seq = "select SEQ_REG_NUM_IN.nextval from dual";
    $list_reg_num_seq = $db -> Select($sql_reg_num_seq);
    $reg_num_next_val = $list_reg_num_seq[0]['NEXTVAL'];

    //виды документов
    $sql_kind = "select * from DIC_DOC_KIND where KIND = 1 order by id";
    ...
    */
    ...
    $list_persons = $db -> Select($sql_persons);
     ...
     $sql_resolution = '';
     $sql_resolution = "select s.id, s.lastname, substr(s.firstname, 1, 1) first,substr(s.middlename, 1, 1) middle, s.email from sup_person s where JOB_SP = 14 and state = 2 order by id asc";
     $list_resolution = $db->select($sql_resolution);

    $today_date = date('d.m.Y');
    $today_date_plus_15 = date("d.m.Y", mktime(0, 0, 0, date('m'), date('d') + 15, date('Y')));

    //$sql_trip = "select * from DOC_TRIP WHERE TRIP_TYPE = 1 order by ID";
    ...
    $sql_trip = "select 
                ...
                from
                    DOC_TRIP dc_trip, 
                    POSITION_GROUP_AND_DOC_TRIP ps_and_trip 
                WHERE 
                    dc_trip.ID = ps_and_trip.DOC_TRIP_ID and
                    ps_and_trip.POS_GROUP = $pos_grp_id and
                    dc_trip.TRIP_TYPE = 1
                order by dc_trip.ID";
    $list_trip = $db -> Select($sql_trip);
?>
