<?php
    ...
    {      
        $empId = $_GET['employee_id'];
        $email = $active_user_dan['mail'];
        $sp = $db->Select("select * from SUP_PERSON where email = '$email'");        
        if($sp[0]['JOB_SP'] !== "11" and $sp[0]['JOB_SP'] != 27){
            if($sp[0]['ID'] !== $empId)
            {
    ...
    //обновить статус леч.пособия
    if(isset($_POST['PAYING_FOR_HELTH_YEAR']))
    {
        $PAYING_FOR_HELTH_YEAR = $_POST['PAYING_FOR_HELTH_YEAR'];
        $PAYING_FOR_HELTH_STATE = $_POST['PAYING_FOR_HELTH_STATE'];

        $employee -> update_health_paying_state($PAYING_FOR_HELTH_YEAR, $PAYING_FOR_HELTH_STATE);
        
    }
    
    // изменения подписанта в доках
    if(isset($_POST['PODPISANT']))
    {
        $podpisant = $_POST['PODPISANT'];
        $positionid_signer = $_POST['position_podpis'];
        $osnovanie = $_POST['osnovanie'];                               
        
        $employee -> update_podpisant($podpisant, $positionid_signer, $osnovanie);
    }
    ...
    if(isset($_POST['NABDAVKA']))
    {
        ...
        $list = $db -> Execute($sql);
        $sql_update_history = "insert into T2_CARD (ID, EVENT_DATE, ID_PERSON, BRANCH_ID, DEPARTMENT, POSITION, ACT_ID, ORDER_DATE, ORDER_NUM, EMP_ID, EVENT_START_DATE) values (SEQ_T2_CARD.nextval, '$today_date', '$empId', '$BRANCH_IDholi', '$DEPARTMENTholi', '$POSITIONholi', '9', '$NABDAVKA_ORDER_DATE', '$ORDER_NUM', '$AUTHOR_ID', '$NABDAVKA_DATE')";
        $update_history = $db -> Execute($sql_update_history);
    }
   ...
   /*     $sql_upd_sup_arc = "BEGIN Copy_rec_person(
        ...
        $sql_update_dismiss = "update sup_person set BANK_ID = '$BANK_ID_change', ACCOUNT_TYPE = '$ACCOUNT_TYPE', ACCOUNT = '$ACCOUNT' where id = $empId";
    ...
    if(isset($_POST['DATE_dismiss']))
    {
        ...
        $DEPARTMENTholi = $_POST['JOB_SPholi'];
        $POSITIONholi = $_POST['JOB_POSITIONholi'];
        $REASON_LAYOFF = $_POST['REASON_LAYOFF'];
    ...
    /*    $sql_upd_sup_arc = "BEGIN Copy_rec_person(
        ...
        $document->sendmail('t.khamitov@gak.kz', 'Уведомление об увольнении сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДата увольнения: $DATE_dismiss");
        $document->sendmail('n.omirbekov@gak.kz', 'Уведомление об увольнении сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL\r\nДата увольнения: $DATE_dismiss");
        $document->sendmail('b.abdisamat@gak.kz', 'Уведомление об увольнении сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL\r\nДата увольнения: $DATE_dismiss");
    ...
    if(isset($_POST['GET_TABLE_WITH_DATA']))
    {
        ...
        $OKLAD = $_POST['OKLAD'];
        $emp_id = $_POST['emp_id'];
        $ORDER_NUM_CHAGE_POS = $_POST['ORDER_NUM_CHAGE_POS'];
        $ORDER_TO_TRANSFER_DATE = $_POST['ORDER_TO_TRANSFER_DATE'];
        $EVENT_START_DATE = $_POST['EVENT_START_DATE'];

        $BRANCHID_sql = "select NAME from DIC_BRANCH where RFBN_ID = $BRANCHID";
        $BRANCHID_list = $db -> Select($BRANCHID_sql);
        if($JOB_SP != 0)
        {
            $JOB_SP_sql = "select NAME from DIC_DEPARTMENT where ID = $JOB_SP";
            $JOB_SP_list = $db -> Select($JOB_SP_sql);
            $JOB_SP_VIEW = $JOB_SP_list[0]['NAME'];
        }else{
            $JOB_SP_VIEW = '';
        }

        //запрос текущий должности
        $sql_current_position = "select JOB_POSITION from sup_person where ID = $emp_id";
        $update_current_position = $db -> Select($sql_current_position);

        $current_pos_id = $update_current_position[0]['JOB_POSITION'];

        //Сюда вставить SQL код с для получения айди руководителя
        $sql_update_job_posit = "update sup_person set BRANCHID = '$BRANCHID', JOB_SP = '$JOB_SP', JOB_POSITION = '$JOB_POSITION', ID_RUKOV = '$ID_RUKOV', OKLAD = '$OKLAD' where id = $emp_id";
        $update_job_posi_list = $db -> Execute($sql_update_job_posit);

        $sql_update_history = "insert into T2_CARD (ID, EVENT_DATE, BRANCH_ID, DEPARTMENT, POSITION, SALARY, ID_PERSON, ACT_ID, OTPR, ORDER_NUM, ORDER_DATE, POSITION_FROM, EVENT_START_DATE) values (SEQ_T2_CARD.nextval, '$ORDER_TO_TRANSFER_DATE', '$BRANCHID', '$JOB_SP', '$JOB_POSITION', '$OKLAD', '$emp_id', '3', '0', '$ORDER_NUM_CHAGE_POS', '$ORDER_TO_TRANSFER_DATE', '$current_pos_id', '$EVENT_START_DATE')";
        $update_history = $db -> Execute($sql_update_history);

        /*
        //закрываем старый табель
        $sql_update_table_state = "update TABLE_OTHER set ACCOUNT_STATE = '0' where EMP_ID = $emp_id";
        $list_update_table_state = $db -> Execute($sql_update_table_state);

        //меняем статус старого табеля
        $sql_update_table_dismiss = "update TABLE_OTHER set VALUE = ' ' where EMP_ID = $emp_id and DAY_DATE between '$ORDER_TO_TRANSFER_DATE' and '31.12.2050'";
        $list_update_table_dismiss = $db -> Execute($sql_update_table_dismiss);

        //создаем новый табель начало
        $work_start_day = $ORDER_TO_TRANSFER_DATE;
        $post_day_list = explode('.', $work_start_day);
        $post_day = $post_day_list[0];

        $post_withuot_day = '.'.$post_day_list[1].'.'.$post_day_list[2];
        create_other_table("$post_withuot_day", $emp_id, $post_day);
        ...
        for($i = $post_day_list[1]+1; $i <= 12; $i++)
        {
            ...
            {
                $post_withuot_day = '.0'.$i.'.'.$post_day_list[2];
                create_other_table_for_this_year("$post_withuot_day", $emp_id, '01');
            }
                else
            {
                $post_withuot_day = '.'.$i.'.'.$post_day_list[2];
        ...
        //создаем новый табель конец
        */

        $sql_branch_name = "select NAME from DIC_BRANCH where RFBN_ID = $BRANCHID";
        $list_branch_name = $db->Select($sql_branch_name);
        if($BRANCHID == '0000'){
                $sql_dep_name = "select NAME from DIC_DEPARTMENT where ID = $JOB_SP";
                $list_dep_name = $db->Select($sql_dep_name);
                $dep_name = $list_dep_name[0]['NAME'];
            }else{
                $dep_name = '';
            }
        $sql_dolzh = "select D_NAME from DIC_DOLZH where ID = $JOB_POSITION";
        ...
        ?>
            <div class="row">
                ...
                <div class="col-lg-6">
                    ...
                    <input disabled="" name="JOB_SP" placeholder="" class="form-control" id="JOB_SP" value="<?php echo $dep_name; ?>">
                </div>
            </div><br />
            <div class="row">
                <div class="col-lg-5">
                ...
                <div class="col-lg-5">
                    ...
                    <input disabled="" name="ID_RUKOV" placeholder="" class="form-control" id="ID_RUKOV" value="<?php echo $ID_RUKOV; ?>">
                </div>
                <div class="col-lg-2">
                    <label class="font-noraml">Оклад</label>
                    <input disabled="" name="OKLAD" placeholder="" class="form-control" id="OKLAD" value="<?php echo $OKLAD; ?>">
                </div>
            </div>
        <?php
        exit;
    ...
    if(isset($_POST['BRANCHID_FOR_POSITIONS'])){
        $BRANCHID = $_POST['BRANCHID_FOR_POSITIONS'];
        //должности
        $sqlPosition .= "select * from DIC_Dolzh where BRANCH_ID = $BRANCHID";
        if($_POST['JOB_SP_FOR_POSITIONS'] != 0){
            $JOB_SP = $_POST['JOB_SP_FOR_POSITIONS'];
            $sqlPosition .= " and ID_DEPARTMENT = $JOB_SP";
        }
    ...
    if(isset($_POST['DATE_BEGINholiReturn'])){
        ...
        $id_peronholireturn = $_POST['ID_PERSONholiReturn'];
        ...
        $sql_update_history = "insert into T2_CARD (ID, EVENT_DATE, ACT_ID, ID_MISS, BRANCH_ID, DEPARTMENT, POSITION, ID_PERSON, EMP_ID, ORDER_NUM, ORDER_DATE) values 
                                  (SEQ_T2_CARD.nextval, '$today_date','8', '$sequance', '$BRANCH_IDholi', '$JOB_SPholi', '$JOB_POSITIONholi', '$empIdEditHoli', '$AUTHOR_ID', '$ORDER_NUMholiReturn', '$ORDER_DATEholiReturn')";
        ...
        $sql_change_state = "update SUP_PERSON set STATE = '2' where id = '$empIdEditHoli'";
        $list_change_state = $db -> Execute($sql_change_state);
        ...
        mail('a.omarov@gak.kz', 'Уведомление об отзыве сотрудника', "ФИО: $lastname $firstname $middlename \r\n Почта: $PERSONAL_EMAIL \r\n Департамент:$DEP\r\n Должность:$DOLZHNOST\r\n Дата отзыва: с $DATE_BEGINholiReturn", 'From: hr@gak.kz');
        mail('n.omirbekov@gak.kz', 'Уведомление об отзыве сотрудника', "ФИО: $lastname $firstname $middlename \r\n Почта: $PERSONAL_EMAIL \r\n Департамент:$DEP\r\n Должность:$DOLZHNOST\r\n Дата отзыва: с $DATE_BEGINholiReturn", 'From: hr@gak.kz');                                                                                                                                                                                                                                     
        exit;
    }
...
<?php
    ...
    if(isset($_POST['ID_PERSONhospEdit'])){
        ...
        echo $listUpdateHospId;
        
        //функция get_inf_from_DB_HOSPITAL() возвращает массив с данными о больнич<img src="C:/Users/b.abdisamat/Desktop/структура2.png" width="1125" height="617" />ными
        $listHOSPITAL = $employee -> get_inf_from_DB_HOSPITAL();
        ...
        $document->sendmail('a.daykarayev@gak.kz', 'Уведомление о больничном сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата больничного: с $DATE_BEGINhospEdit по $DATE_ENDhospEdit");
        $document->sendmail('b.abdisamat@gak.kz', 'Уведомление о больничном сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата больничного: с $DATE_BEGINhospEdit по $DATE_ENDhospEdit");
   //     mail('b.abdisamat@gak.kz', 'Уведомление о больничном сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата больничного: с $DATE_BEGINhospEdit по $DATE_ENDhospEdit", 'From: hr@gak.kz');
   //     mail('a.daykarayev@gak.kz', 'Уведомление о больничном сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата больничного: с $DATE_BEGINhospEdit по $DATE_ENDhospEdit", 'From: hr@gak.kz');
        exit;                                        
    }
    //редактирование
...
<?php
    ...
    {                                
        ...
        {
            //проверка количества дней, раскомментить
        ...
        {
            $this_year = date("Y");
            $sql_health_paying_state = "update PERSON_HOLI_PAY set STATE = '1' where EMP_ID = '$ID_PERSONholi' and YEAR = '$this_year'";
            $list_health_paying_state = $db -> Execute($sql_health_paying_state);
        }
        ...
        $DOLZHNOST = $v['D_NAME'];
        $job_sp = $v['JOB_SP'];
        $job_pos = $v['JOB_POSITION'];
        
         $email_to = array();
        $list_all_employes_of_head = $db->select("select * from sup_person where state = 2 and branchid = '0000'"); 
        foreach($list_all_employes_of_head as $x=>$y){
            $file_id = $y['EMAIL'];
            array_push($email_to, $file_id);
        }
        
        $list_vacation_notification = $db->select("select * from sup_person where state = 2 and job_position in (1,2,3,4,5,10,14,18,21,25,28,31,35,37,41,48,50,76,167,168,182,211,214,321,383,385,388,400,423)");
                
        foreach($list_vacation_notification as $kk=>$vv)
        {
            if($vv['JOB_POSITION'] == $job_pos){
                        $document->sendmail($email_to, 'Уведомление об отпуске сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата отпуска: с $DATE_BEGINholi по $DATE_ENDholi"); 
            }   
             else{   
                        $document->sendmail('a.daykarayev@gak.kz', 'Уведомление об отпуске сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата отпуска: с $DATE_BEGINholi по $DATE_ENDholi");
                        $document->sendmail('d.karlygasheva@gak.kz', 'Уведомление об отпуске сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата отпуска: с $DATE_BEGINholi по $DATE_ENDholi");           
        }         
        }                           
...
<?php
    ...
    if(isset($_POST['delete_det_id'])){
        ...
        $sql_trip_det_delete = "delete TRIP_FROM_TO where ID = $delete_det_id";  
            
        $list_trip_det_delete = $db -> Select($sql_trip_det_delete);
    ...
    if(isset($_POST['TRIP_DETAIL_ID'])){
        ...
        $sql_trip = "insert into TRIP_FROM_TO (ID, TRIP_ID, FROM_PLACE, TO_PLACE, TRANSPORT) values 
                             (SEQ_TRIP_FROM_TO.nextval, '$TRIP_DETAIL_ID', '$FROM_PLACE', '$TO_PLACE', '$TRANSPORT')";  
    ...
    if(isset($_POST['DATE_BEGINtrain'])){
        ...
        $employee = new Employee($ID_PERSONtrain);
        
        $employee -> add_training($ID_PERSONtrain, $DATE_BEGINtrain, $DATE_ENDtrain, $NAME_TRAIN, $NAME_TRAIN_KAZ, $CNT_DAYtrain, $SUM_TRAIN, $ADRESS_TRAIN, $ADRESS_TRAIN_KAZ );
    ...
    if(isset($_POST['DATE_BEGINtrip'])){
        ...
        $document->sendmail('a.daykarayev@gak.kz', 'Уведомление о командировке сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата командировки: с $DATE_BEGINtrip по $DATE_ENDtrip");   
        $document->sendmail('b.abdisamat@gak.kz', 'Уведомление о командировке сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата командировки: с $DATE_BEGINtrip по $DATE_ENDtrip");     
    //    mail('a.daykarayev@gak.kz', 'Уведомление о командировке сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата командировки: с $DATE_BEGINtrip по $DATE_ENDtrip", 'From: hr@gak.kz');
    //    mail('b.abdisamat@gak.kz', 'Уведомление о командировке сотрудника', "ФИО: $lastname $firstname $middlename \r\nПочта: $PERSONAL_EMAIL \r\nДепартамент:$DEP\r\nДолжность:$DOLZHNOST\r\nДата командировки: с $DATE_BEGINtrip по $DATE_ENDtrip", 'From: hr@gak.kz');
...
<?php
    ...
    if(isset($_POST['empIdTrivial'])){
        ...
        $CONTRACT_JOB_DATE = $_POST['CONTRACT_JOB_DATE'];
        $BRANCHID = $_POST['BRANCHID'];
        $JOB_SP = $_POST['JOB_SP'];
        $JOB_POSITION = $_POST['JOB_POSITION'];
        $ID_RUKOV = $_POST['ID_RUKOV'];
    ...
    //возвращает массив с обучением сотрудников
    $list_train = $employee -> get_training();
    
    $list_signer = $db->select("select sp.*, d.*, st.id podpis_id, st.lastname podpis_lastname, ST.FIRSTNAME podpis_firstname, st.middlename podpis_middlename from sup_person sp , dic_dolzh d, sup_person st where SP.JOB_POSITION = d.id and D.ID_PODPIS = st.id  and sp.id = $empId");
    ...
    //роли
    $sqlRole = "select * from DIR_ROLE order by id";
    $listRole = $db -> Select($sqlRole);

    //статусы документов
    $sql_state = "select * from DIC_PERSON_DOCS_STATE order by id";
    ...
    //лечебное пособие на этот год
    $sql_holi_for_health_paying = "select * from PERSON_HOLI_PAY where EMP_ID = '$empId' and year = '$date' ";
    $list_holi_for_health_paying = $db->Select($sql_holi_for_health_paying);
        
    //причины увольнения
    $sql_layoff_reason = "select * from DIC_REASON_LAYOFF order by ID";
    $list_layoff_reason = $db -> Select($sql_layoff_reason);
    
    if(count($list_holi_for_health_paying)){
        if($list_holi_for_health_paying[0]['STATE'] == '1')
        {
            $sqlVID_HOLIDAYS = "select * from DIC_VID_HOLIDAYS where ID != 1 order by id";
            $listVID_HOLIDAYS = $db -> Select($sqlVID_HOLIDAYS);
        }
    }
    ...
    $sql_holy_period = "select * from PERSON_HOLYDAYS_PERIOD where PERSON_ID = $empId and PERIOD_START < '$today_date' order by id";
    ...
    $timesheet_date_end = '';
    if(isset($_POST['timesheet_date_start'])){
