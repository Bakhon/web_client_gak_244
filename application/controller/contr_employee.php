<?php
    ...
    $sqlDIC_DOLZH = "select * from DIC_DOLZH order by id";
    $listDIC_DOLZH = $db -> Select($sqlDIC_DOLZH);

    $sqlDIC_DOLZH = "select * from DIC_DOLZH order by id";
    $listDIC_DOLZH = $db -> Select($sqlDIC_DOLZH);

    $sqlDIC_DEPARTMENT = "select * from DIC_DEPARTMENT order by id";
    ...
    {
        //табельный номер
        $sql_getnum = "begin :rs := getnum('TAB_N'); end;";
        $TAB_NUM_ARRAY = $db -> ExecuteReturn($sql_getnum, array('rs'));

        $TAB_NUM = $TAB_NUM_ARRAY['rs'];
        ...
        $seqArray = $db -> Select("select SUP_PERSON_SEQ.nextval from dual");
        ...
        $sql = "insert into sup_person (ID, TAB_NUM, IIN, LASTNAME, FIRSTNAME, MIDDLENAME, BIRTHDATE, SEX, BIRTH_PLACE, 
        NACIONAL, DATE_POST, STATE, JOB_SP, JOB_POSITION, OKLAD, DOCNUM, DOCTYPE, DOCDATE, DOCPLACE, FACT_ADDRESS_COUNTRY_ID, FACT_ADDRESS_CITY, FACT_ADDRESS_STREET, FACT_ADDRESS_BUILDING, FACT_ADDRESS_FLAT, BRANCHID, MOB_PHONE, LASTNAME2, FIRSTNAME2, PERSONAL_EMAIL, CONTRACT_JOB_NUM, EMAIL) values
                                        ($sequance, 
                                        '$TAB_NUM', 
                                        '$IIN', 
                                        '$lastname', 
        ...
        $sql_update_history = "insert into T2_CARD (ID, EVENT_DATE, BRANCH_ID, DEPARTMENT, POSITION, SALARY, ID_PERSON, ACT_ID) values (SEQ_T2_CARD.nextval, '$DATE_POST', '$BRANCHID', '$JOB_SP', '$JOB_POSITION', '$OKLAD', '$sequance', '1')";
        ...
        //отправка ВНД на ознакомление
         ...
         $id_vnd = $list_seq[0]['NEXTVAL'];
        
        
         $sql_i = "INSERT INTO DOCUMENTS(ID, TYPE, HEAD_TEXT, SHORT_TEXT,   STATE,  SENDER, SENDER_MAIL, DATE_START, KIND) VALUES ($id_vnd, 0, 'ВНД на ознакомление при приеме на работу', 'ВНД на ознакомление при приеме на работу', 16,  'СУП', 'a.ibrayeva@gak.kz', '$today_date', 15)";
         $sql_insert = $db->Execute($sql_i);
            
            
         $sql = "insert into DOC_RECEIP_USAGE (ID, MAIL_ID,  RECIEP_MAIL, STATE, SENDER_MAIL, RECIEP_DATE, RECIEP_TIME, DESTINATION, LINK_FILE) values (SEQ_DOC_RECIEPMENTS.NEXTVAL, '$id_vnd', '$PERSONAL_EMAIL', '0', 'a.ibrayeva@gak.kz', '$today_date', '$now_time', '1', '10761')";               
      ...
      //  mail("$PERSONAL_EMAIL", 'Уведомление в СЭД', "Вам поступило письмо на ознакомление. Для ознакомления пройдите по ссылке http://192.168.5.244/on_inbox_new.", "From: Система электронного документооборота");
        $document->sendmail($PERSONAL_EMAIL, 'Уведомление в СЭД', "Вам поступило письмо на ознакомление. Для ознакомления пройдите по ссылке http://192.168.5.244/on_inbox_new");
        ...
        if($BRANCHID == '0000') {
        $document->sendmail('t.khamitov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('n.omirbekov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('a.saleev@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");    
        $document->sendmail('b.abdisamat@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('a.auganbaeva@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('d.kainarov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('a.daykarayev@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");        
  /*      mail('a.omarov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        mail('n.omirbekov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        mail('a.saleev@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        mail('b.abdisamat@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        mail('a.auganbaeva@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        mail('d.kainarov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        mail('a.daykarayev@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        */
        }
        else {
        //mail('i.akhmetov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL", 'From: hr@gak.kz');
        $document->sendmail('t.khamitov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('n.omirbekov@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('a.saleev@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        $document->sendmail('b.abdisamat@gak.kz', 'Заявка на создание нового сотрудника', "ФИО: $lastname $firstname $middlename \r\nФилиал: $branch_name \r\nДепартамент: $dep_name \r\nДолжность: $dolzh_name \r\nПочта: $PERSONAL_EMAIL");
        ...
        */
        ...
        //кнопка для документооборота новому сотруднику
        $sql_dolzh = "insert into EVENT_POSIOTION (POS_ID, EVENT_ID) values ('$JOB_POSITION', '1')";
        $list_dolzh = $db -> Select($sql_dolzh);
        $sql_dolzh = "insert into EVENT_POSIOTION (POS_ID, EVENT_ID) values ('$JOB_POSITION', '3')";
        $list_dolzh = $db -> Select($sql_dolzh);

         // добавление периода леч пособия
         $period = array('2019','2020', '2021', '2022', '2023');
         
        $sql_check = "select count(*) c from person_holi_pay where year = $period and emp_id = $sequance";
        $list_check = $db->select($sql_check);       
        
        if($list_check[0]['C'] <= 0) {
        
        $sql = "INSERT INTO PERSON_HOLI_PAY (YEAR, STATE, EMP_ID) VALUES ($period, 0, $sequance)";
        $list_sql = $db->Execute($sql);
        }
                                         
        //отправляем данные в ШЭП
        $c = new E_CONTRACTS();
        $c->contract($sequance);
        $msqs = $c->message;
        
        /*Добавляем данные для автоматического создания пользователя в таблице GS_EMP на 3 серваке (добавил данный код Салеев А.Р. 12.02.2019 года)*/
        $db3 = new DB3();        
        $q1 = $db3->Select("select SEQ_GS_EMP.nextval id from dual");
        $id = $q1[0]['ID']; 
        $role = $db->Select("select id_role_ins from DIC_DOLZH where id = $JOB_POSITION");
        $id_role = $role[0]['ID_ROLE_INS'];
                
        $email_user = str_replace('@gak.kz', '', $PERSONAL_EMAIL);
        $fio_user = $lastname.' '.strtoupper(substr($firstname, 1, 1)).'.'.strtoupper(substr($middlename, 1, 1));
        $branch = $JOB_SP;
        
        //Заглушка на департамент корпоративных продаж
        if($JOB_SP == '1'){
            $branch = '1701';
        }
        
        $sqls3 = "INSERT INTO GS_EMP (EMP_ID, BRANCH_ID, VCUSERNAME, VCPASSWORD, BDISABLED, VCMAIL, NAME, PROGR, 
        LASTNAME, FIRSTNAME, LOGIN, ON_PA) 
        VALUES ($id, '$JOB_SP', '$email_user', '0000000', '1', '$PERSONAL_EMAIL', '$fio_user', 1, '$lastname', '$firstname', '$email_user', 0)";
        $db3->Execute($sqls3);
        
        if($id_role !== '0'){
            $sqls4 = "INSERT INTO EMP_ROLE (ID, EMP_ID, ROLE) VALUES (SEQ_EMP_ROLE.nextval, '$id', '$id_role')";
            $db3->Execute($sqls4);
        }
        /*------------------------------------------------------------------------------------------------------------------------------*/
        
        header('Location: edit_employee?employee_id='.$sequance);
    }
    ...
    if(isset($_POST['iin_for_check']))
    {
        ...
        $sql_to_check_iin = "select * from sup_person where iin  = $IIN";
