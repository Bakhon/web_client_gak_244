<?php
    $db = new DB();
    $sql_sender_mail = "SELECT * FROM DIC_META ORDER BY ID";
    $list_sender_mail = $db -> Select($sql_sender_mail);

    foreach($list_sender_mail as $k => $v)
    {
        $sql_sender_mail = "INSERT INTO DIC_META
        (`ID`, `META`, `VARIABLE`, `NAME_RU`) VALUES 
        ('".$v['ID']."','".$v['META']."','".$v['VARIABLE']."','".$v['NAME_RU']."');";
        echo $sql_sender_mail.'<br /><br />';
    }
    exit;
/*
    ...
    if(isset($_POST['CREATE_MAIL']))
    {
        ...
        if($state != '0')
        {
                ...
                else
            {
                ...
                foreach ($_POST['RECIPIENT_AGREEMENT'] as $key=>$reciep_mail)
                {
                    $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_AGREEMENT (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, POST_DATE, POST_TIME, DESTINATION, AGREEMENT_TYPE) VALUES (SEQ_DOC_RECIEPMENTS_AGREEMENT.NEXTVAL, '$id', '$reciep_mail', '0', '$emp_mail', '$today_date', '$now_time', '$DESTINATION', '1')";
                    $list_reciep = $db -> Select($sql_reciep);
                    if($reciep_mail == 'g.amerkhojayev@gak.kz') $reciep_mail = 'z.kassenova@gak.kz';
                    mail("$reciep_mail", 'Увемдомление СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на согласование с коротким описанием '$SHORT_TEXT'. Для прочтения пройдите по ссылке http://192.168.5.244/on_inbox_new", "From: Система электронного документооборота");
                }
            }
    ...
    function add_agreement_person($RECIPIENT_AGREEMENT)
    {
        echo $RECIPIENT_AGREEMENT.'<br />';
    }
?>
