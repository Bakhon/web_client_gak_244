<?php
        ...
        if($state != '0')
        {
            if($trip_id == '5')
            {
                ...
                foreach ($_POST['RECIPIENT_AGREEMENT'] as $key=>$reciep_mail)
                {
                    ...
                    $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_AGREEMENT (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, POST_DATE, POST_TIME, DESTINATION, AGREEMENT_TYPE) VALUES (SEQ_DOC_RECIEPMENTS_AGREEMENT.NEXTVAL, '$id', '$reciep_mail', '$state_rec', '$emp_mail', '$today_date', '$now_time', '14', '2')";
                    $list_reciep = $db -> Select($sql_reciep);
                    //mail("$reciep_mail", 'Увемдомление СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо на согласование с коротким описанием '$SHORT_TEXT'. Для прочтения пройдите по ссылке http://192.168.5.244/mail_detail?doc_id=$id&get_file=$id&rec_id=$rec_id&dest_id=$DESTINATION&step_id=16&next_step_id=25&state_id=2&prev_step_id=0&table_name_eng=DOC_RECIEPMENTS_AGREEMENT", "From: Система электронного документооборота");
                    $i++;
                }
            }
                else
            {
                ...
                foreach ($_POST['RECIPIENT_AGREEMENT'] as $key=>$reciep_mail)
                {
                    ...
                    $list_seq = $db -> Select($sql_id);
                    $rec_id = $list_seq[0]['NEXTVAL'];
                    $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_AGREEMENT (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, POST_DATE, POST_TIME, DESTINATION, AGREEMENT_TYPE) VALUES ($rec_id, '$id', '$reciep_mail', '0', '$emp_mail', '$today_date', '$now_time', '$DESTINATION', '1')";
                    $list_reciep = $db -> Select($sql_reciep);
            ...
            {
                $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_ASSIGNMENT (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, POST_DATE, POST_TIME, DESTINATION) VALUES (SEQ_DOC_RECIEPMENTS_ASSIGNMENT.NEXTVAL, '$id', '$reciep_mail', '0', '$emp_mail', '$today_date', '$now_time', '$DESTINATION')";
                $list_reciep = $db -> Select($sql_reciep);
            }
            ...
            {
                $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_REGISTRATION (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, POST_DATE, POST_TIME, DESTINATION) VALUES (SEQ_DOC_RECIEPMENTS_REGISTRATI.NEXTVAL, '$id', '$reciep_mail', '0', '$emp_mail', '$today_date', '$now_time', '$DESTINATION')";
                $list_reciep = $db -> Select($sql_reciep);
            }
            ...
            {
                $sql_reciep = "INSERT INTO DOC_RECIEPMENTS_RESOLUTION (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, POST_DATE, POST_TIME, DESTINATION) VALUES (SEQ_DOC_RECIEPMENTS_RESOLUTION.NEXTVAL, '$id', '$reciep_mail', '0', '$emp_mail', '$today_date', '$now_time', '$DESTINATION')";
                $list_reciep = $db -> Select($sql_reciep);
            }
            ...
            foreach ($_POST['RECIPIENT'] as $key=>$reciep_mail)
            {
                ...
                $sql_reciep = "INSERT INTO DOC_RECIEPMENTS (ID, MAIL_ID, RECIEP_MAIL, STATE, SENDER_MAIL, POST_DATE, POST_TIME, DESTINATION) VALUES (SEQ_DOC_RECIEPMENTS.NEXTVAL, '$id', '$reciep_mail', '$state_rec', '$emp_mail', '$today_date', '$now_time', '$DESTINATION')";
                $list_reciep = $db -> Select($sql_reciep);
                if($reciep_mail == 'g.amerkhojayev@gak.kz') $reciep_mail = 'z.kassenova@gak.kz';
                mail("$reciep_mail", 'Увемдомление СЭД', "К Вам, в СЭД, от сотрудника $emp_fio пришло письмо с коротким описанием '$SHORT_TEXT'. Для прочтения пройдите по ссылке http://192.168.5.244/mail_detail?doc_id=$id&get_file=$id&rec_id=$rec_id&dest_id=$DESTINATION&step_id=81&next_step_id=84&state_id=4&prev_step_id=0&table_name_eng=DOC_RECIEPMENTS", "From: Система электронного документооборота");
            }
    ...
    //сотрудники
    $sql_persons = "select dep.NAME DEP_NAME, triv.EMAIL, triv.LASTNAME ||' '|| triv.FIRSTNAME ||' '|| triv.MIDDLENAME fio, dolzh.D_NAME from sup_person triv, DIC_DOLZH dolzh, DIC_DEPARTMENT dep where triv.JOB_POSITION = dolzh.ID and triv.JOB_SP = dep.ID and triv.STATE in (2, 7) order by fio";
