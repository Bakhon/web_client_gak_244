 ...
 $id = 0;
    ...
    foreach($form as $k=>$v){           
        ...
        if(trim($text) == '<div class="pagebreak"> </div>'){
            ...
            $html .= '<div style="width: '.$width.'%; float: '.$fl.';">';
        }        
        ...
        foreach($p as $t=>$d){            
            ...
            if($d['TABLE_NAME'] == $table1){
                ...
                $main_sql = '';
                $sql = CreateSQL($table1, $d['TABLE_NAME'], $d['COL_NAME'], $main_col, $val);                
                $rp = $db->Select($sql);
                if(!$rp){      
                    $sql = CreateSQL2($table1, $d['TABLE_NAME'], $d['COL_NAME'], $main_col, $val);                    
                    $rp = $db->Select($sql);  
                    $text = str_replace($span, $rp[0]["SP"], $text);
                }else{
                    $text = str_replace($span, $rp[0]["SP"], $text);  
