<?php
    ...
    $sql = "select * from REPORTS_HTML where id = ".$rep['ID'];
    $rep = $db->Select($sql);
    ...
    $db->ClearParams();
    $dan = $db->Select("select * from REPORTS_HTML where id = $r");
    ...
    /**
     * Переделываем входящие переменные из запроса в селекте 
     * Пример:
     * select * from contracts where cnct_id = :cnct_id
     * в случаи если был GET запрос cnct_id = 123 тогда селект изменится на
     * select * from contracts where cnct_id = '123'
    */        
?>