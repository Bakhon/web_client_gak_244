<?php
    ...
    array_push($css_loader, 
        ...
        'styles/css/plugins/Flipclock/flipclock.css',
      ...
      $sql_list_project = "Select * from PROJECT where STATE = 2 order by id";
...
if($_GET['proj_id'])
    {
        ...
        $que = "select * from (select * from question_p order by dbms_random.value ) where rownum <= 40 and id_project = $id_project order by id ASC ";
   //   $que = "select * from question_p where id_project = $id_project";
        $l_que = $db -> select($que);
      ...
      if(isset($_POST['endtest'])){
          ...
          $q = $db->Select("select * from sup_person where email = '$em' and state in (2,3) "); 
           ...
           $list_insert_users_res = $db -> execute($sql_insert);
              ...
              $sql_list_project = "Select * from PROJECT where id = $id_project order by id";
              $list_proj = $db->Select($sql_list_project);
              
              foreach($list_proj as $k=>$v){
                 if($v['ID_TYPE'] == '1')
                 {                                       
                    header("Location: res_full_test_client?project_id=$id_project&user_id=$id_user");   
                 }
               else
                 {
                    header("Location:/");
