<?php
    ...
    if(isset($_POST['id_table'])){
        ...
        $timesheet_date_start = $_POST['timesheet_date_start'];
        $timesheet_date_end = $_POST['timesheet_date_end'];
        $sql_guys = "select dolzh.D_NAME, trivial.JOB_POSITION, trivial.FIRSTNAME, trivial.MIDDLENAME, trivial.LASTNAME, trivial.ID from sup_person trivial, DIC_DOLZH dolzh where dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id_for_table' and trivial.STATE = 2 order by JOB_POSITION";
    ...
    $sql_for_cur = "select dolzh.D_NAME, SUBSTR(trivial.FIRSTNAME, 1, 1) ||'. '|| SUBSTR(trivial.MIDDLENAME, 1, 1) ||'. '|| TRIVIAL.LASTNAME FIO from curators cur, sup_person trivial, DIC_DOLZH dolzh where TRIVIAL.ID = CUR.CURATORS_ID and DOLZH.ID = TRIVIAL.JOB_POSITION and CUR.DEP_ID = $dep_id";
