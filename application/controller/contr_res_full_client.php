 ...
 /*    foreach($list_users_res as $n => $m) { 
    ...
    */
    ...
    $list_department = $db -> select($sql_dic_department);
     ...
     $list_res = $db -> select("select u.id_user, d.name, p.id from USERS_RESULT u, sup_person sp, dic_department d, project p where u.id_user = sp.id and u.ID_PROJECT = $project_id and SP.JOB_SP = d.id and u.id_project = p.id group by u.id_user, d.name, p.id");
