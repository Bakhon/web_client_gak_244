...
*/
    ...
    if(isset($_POST['pst']))
    {
        ...
        if($dan['ID'] == 0)
        {
            ...
            //VALUES (SEQ_REPORT_HTML_OTHER.nextval, '$HTML_TEXT', '$pos', '$num', '$title', '$id_otchet')";

            $sql = "INSERT INTO REPORT_HTML_OTHER (ID, HTML_TEXT, POSITION, NUM_PP, TITLE, ID_OTCHET) 
                    VALUES (SEQ_REPORT_HTML_OTHER.nextval, EMPTY_CLOB(), '$pos', '$num', '$title', '$id_otchet')
                    RETURNING HTML_TEXT INTO :HTML_TEXT";
            ...
            else
        {
            ...
            SET   
            HTML_TEXT = EMPTY_CLOB(),
            POSITION  = '$pos',
            NUM_PP    = '$num',
            TITLE     = '$title'        
            where ID = $id RETURNING HTML_TEXT INTO :HTML_TEXT";
            //$al = $db->Execute($sql);
        }
        $al = $db->AddClob($sql, $p);
        /*
    ...
    if(isset($_POST['TITLE_TEXT'])){
        ...
        VALUES (SEQ_REPORT_HTML_OTHER.nextval, 0, :html, 1, '$position', $numpp, '$titleText', $idOtchet)";
        $q->ClearParams();
    ...
    if(isset($_POST['editTITLE_TEXT'])){
        ...
        $sql = "update report_html_other set HTML_TEXT = :html, position = '$position', num_pp = $numpp, title = '$titleText', id_otchet = '$idOtchet' where id = $id";
        $q->ClearParams();
        $bool = $q->ExecProc($sql, $array);
        header('location: standart_contracts?sicid='.$_GET['sicid']);
    }
