<?php
    ...
    */       
    ...
    if(isset($_POST['MIS_DESCRIPTION']))
    {
        ...
        $jabber = new JABBER();
        $jabber->send_message("b.abdisamat@gak.kz", "Ошибка в системе управления персоналом");
    ...
    if(isset($_POST['dep_id_for_table']))
    {
        ...
        $sql_guys = "select dolzh.D_NAME, trivial.JOB_POSITION, trivial.FIRSTNAME, trivial.MIDDLENAME, 
        trivial.LASTNAME||' '||substr(trivial.FIRSTNAME, 1, 1)||'. '||substr(trivial.MIDDLENAME, 1, 1)||'.' LASTNAME, 
        trivial.ID from sup_person trivial, DIC_DOLZH dolzh where (dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id_for_table' and trivial.STATE = 2) OR (dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id_for_table' and  trivial.STATE = 9) OR (dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id_for_table' and  trivial.STATE = 3) OR (dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id_for_table' and  trivial.STATE = 4) OR (dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id_for_table' and  trivial.STATE = 5) OR (dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id_for_table' and  trivial.STATE = 6) order by JOB_POSITION";
        
        $sql_guys = "select dolzh.D_NAME,
               trivial.JOB_POSITION,
               trivial.FIRSTNAME,
               trivial.MIDDLENAME,
               trivial.LASTNAME || ' ' || substr(trivial.FIRSTNAME, 1, 1) || '. ' ||
               substr(trivial.MIDDLENAME, 1, 1) || '.' LASTNAME,
               trivial.ID
          from sup_person trivial, DIC_DOLZH dolzh
         where 
         dolzh.ID = trivial.JOB_POSITION
         and trivial.STATE in(2, 3, 9, 4, 5, 6) 
         and JOB_SP = '$dep_id_for_table'  
         and TRIVIAL.DATE_POST <= to_date('$timesheet_date_end', 'dd.mm.yyyy') 
         order by JOB_POSITION";
        
        //echo $sql_guys;
        if($_POST['branch_id'] != 0){
            $branch_id = $_POST['branch_id'];
            $sql_guys = "select LASTNAME||' '||substr(FIRSTNAME, 1, 1)||'. '||substr(MIDDLENAME, 1, 1)||'.' LASTNAME, ID from sup_person where BRANCHID = '$branch_id'";
            $dep_id_for_table = '';
        }
    ...
    {
        $dep_id_for_table = $dep_id;
        $branch_id = '0000';
        $MISS_PERSON_ID = $_POST['MISS_PERSON_ID'];
        $timesheet_date_start = $_POST['timesheet_date_start'];
        $timesheet_date_end = $_POST['timesheet_date_end'];
        $sql_guys = "select dolzh.D_NAME, trivial.JOB_POSITION, trivial.FIRSTNAME, trivial.MIDDLENAME, trivial.LASTNAME, trivial.ID from sup_person trivial, DIC_DOLZH dolzh where trivial.ID = $MISS_PERSON_ID and dolzh.ID = TRIVIAL.JOB_POSITION order by JOB_POSITION";
        $list_guys = $db -> Select($sql_guys);
        $sql_guy = "select WEEK_DAY from TABLE_OTHER where DAY_DATE between '$timesheet_date_start' and '$timesheet_date_end' and emp_id = 1481 and ACCOUNT_STATE = '1' order by DAY_DATE";        
        $list_guy = $db -> Select($sql_guy);
    }
    ...
    $sql_dep_guys = "select dolzh.D_NAME, trivial.JOB_POSITION, trivial.FIRSTNAME, trivial.MIDDLENAME, trivial.LASTNAME, trivial.ID from sup_person trivial, DIC_DOLZH dolzh where dolzh.ID = trivial.JOB_POSITION and JOB_SP = '$dep_id'";
    $list_dep_guys = $db -> Select($sql_dep_guys);

    $sql_for_dir = "select DOLZH.D_NAME, SUBSTR(pers.FIRSTNAME, 1, 1) ||'. '|| SUBSTR(pers.MIDDLENAME, 1, 1) ||'. '|| pers.LASTNAME FIO from SUP_PERSON pers, DIC_DOLZH dolzh where pers.state = 2 and DOLZH.ID = PERS.JOB_POSITION and pers.EMAIL = '$mail'";                        
