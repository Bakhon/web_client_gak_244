...
class FORMS
{
    public function InputText($col, $label, $name, $id, $placeholder, $class, $value, $readonly = false, $other_params = false){
        $other_text = '';
        $r = '';
    ...
    public function inputDate($col, $label, $name, $id, $class, $value, $readonly = false, $other_params = false, $required = '')
    {
        $other_text = '';
        $r = '';
        if($other_params !== false){
        ...
        return '<div class="col-lg-'.$col.'">
                            <div class="form-group">
                                ...
                                <div class="input-group date">
                                    ...
                                    <input name="'.$name.'" '.$r.' '.$other_text.' id="'.$id.'" autocomplete="off" type="text" class="'.$class.'" data-mask="99.99.9999" value="'.$value.'" '.$required.'>
                                </div>
                            </div>
                        </div>';
    }
    
    public function FormGroup($lw, $ltext, $rw, $rtext)
    {
        if(trim($rtext) !== '')
        {
    ...
    public function FormHorizontalEdit($lc, $ic, $label, $name, $value, $help = '', $placeholder, $id, $required)
    {
    ...
    public function FormHorizontalRadio($lc, $ic, $checksName,  $checkArray)
    {
        $form = '<div class="form-group">
    ...
    public function FormHorizontalCheck($lc, $ic, $label, $name, $id, $check = '')
    {        
        $c = '';
    ...
    public function FormHorizontalCheckList($lc, $ic, $checksName,  $checkArray)
    {        
        $form = '<div class="form-group">
            <label class="col-lg-'.$lc.' control-label"></label>
            <div class="col-lg-'.$ic.'">';
    ...
    public function FormHorizontalSelect($lc, $ic, $label, $name, $options, $other_class = '', $selected_id = '')
    {
        $class = '';
    ...
    public function FormHorizontalSelectRep($label, $name, $options, $other_class = '', $selected_id = '')
    {
        $class = $other_class;
        if($other_class !== 'form-control'){
            $class = 'select2_demo_1 form-control chosen-select';
        }        
        $form = '<div class="col-lg-12"><div class="form-group">
            <label class="font-noraml">'.$label.'</label>            
            <select class="'.$class.'" name="'.$name.'">';
        foreach($options as $k=>$v){ 
            $id = $v['CODE'];
            ...
            $form .= '<option value="'.$id.'">'.$v['NAME'].'</option>';
        }
        $form .= '</select></div></div>';     
        return $form;
    }
    ...
    public function formGroup2($lw, $utext, $dtext, $view = true){                                    
        static $actCount = 0;
        if(trim($dtext) != '')
        {
        ...
        /*
        public function TableForms($tdarray)
        {
        ...
        /*/
    public function tableForms($sername, $name, $patronymic, $dateOfBirth, $share, $sik, $rnn, $iin, $address){            
    ...
    public function formGroupForDate($lw, $utext, $dtext, $etext){                            
         if(trim($dtext) != '')
             {
        ...
        public function ContractsMenu($size, $cnct, $paym_code){
            return '
        ...
        public function ModalContainer($id, $title, $head_text, $html, $sav_js_function, $icheck = '')
        {
            $form = '
