<?php
	class FTP
    {
        ...
        public function __construct($server = FTP_SERVER, $user = FTP_USER, $pass = FTP_PASS)
        {
            $this->server = $server;
            $this->username = $user;
            $this->password = $pass;
        ...
        public function check_path($path)
        {
            ...
            //$p = ftp_nlist($this->ftp_ses, "/$path");
            $p = ftp_site($this->ftp_ses, "CHMOD 0600 /$path");            
            if($p == true){
                return true;
            }else{
                return false;    
        ...
        {
            //return ftp_fput($this->ftp_ses, $ftp_path.$filename, $local_file, FTP_BINARY);
        ...
        public function uploadfile2($ftp_path, $filename, $local_file)
        {            
            $ss = '';
            $ret = ftp_nb_put($this->ftp_ses, $ftp_path.$filename, $local_file, FTP_BINARY, FTP_AUTORESUME);
            while ($ret == FTP_MOREDATA){
                $ss .= ".";
                $ret = ftp_nb_continue($this->ftp_ses);
            }
            if ($ret != FTP_FINISHED) {
                $ss .= "При загрузке файла произошла ошибка...";
                return false;                
            }
        ...
        {
            if (ftp_delete($this->ftp_ses, $filename)) {
                return True;
            } else {
                return false;
            }
