<?php
	class SQLS
    {
        ...
        public function OSNS($cnct_id, $db)
        {
            $sql_body = "
            select 
            ...
            foreach($ds as $k=>$v){
                ...
                select 
                    ...
                    r.*,
                    (select B.CONTRACT_NUM from bordero_contracts b, bordero_contracts_list l where  L.ID_CONTRACTS = B.ID and b.id = (select max(id_contracts) from bordero_contracts_list where CNCT_ID = r.cnct_id)
                    group by B.CONTRACT_NUM) bordero_num,
                    (select max(B.id) from bordero_contracts b, bordero_contracts_list l where L.ID_CONTRACTS = B.ID and L.CNCT_ID = r.cnct_id) bordero_id
        ...
        public function ChartRegion3($region)
        {                
