                            ...
                            <form method="post" target="_blank" action="print_timesheet">                            
                                <textarea hidden="" name="content">
                                    <div style="text-align: center;">
                                        <strong>
                                            <h3>Завершенные поручения на <?php echo $today_date.' (время: '.$now_time.')'; ?></h3>
                                        </strong>
                                    </div>
                                    <table>
                                        <thead>
                                                ...
                                                foreach($list_reciep as $k => $v)
                                        {
                                            $doc_id = $v['ID'];
                                            $sql_inbox_detail .=
                                                        "select doc.*, TRIVIAL.JOB_SP, DEPART.NAME dep_name, DEPART.SHORT_NAME SHORT_NAME from DOC_ASSIGN_RECIEPMENTS doc, SUP_PERSON trivial, DIC_DEPARTMENT depart where doc.MAIL_ID = '$doc_id' and TRIVIAL.EMAIL = doc.RECIEPMENTS_MAIL and DEPART.ID = TRIVIAL.JOB_SP";
                                            $list_reciep_detail = $db -> Select($sql_inbox_detail);
                                            //echo '<pre>';
                                            //print_r($list_reciep_detail);
                                            //echo '<pre>';
