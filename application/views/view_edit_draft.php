            ...
            <div class="mail-box">
                ...
                <form enctype="multipart/form-data" class="form-horizontal" method="POST" id="main_form">
                    ...
                    <div hidden="" class="form-group"><label class="col-sm-2 control-label">статус:</label>
                        <div class="col-sm-10">
                            <input class="form-control" value="0" id="STATE" name="STATE"/>
                        </div>
                    </div>
                    <div class="form-group" <?php if($trip_id == '3'){$required_state = '';}else{$required_state = 'hidden="" style="display: none;"';} //echo $required_state; ?>><label class="col-sm-2 control-label">Согласующие:</label>
                        <div class="col-sm-10">
                            <select name="RECIPIENT_AGREEMENT[]" id="RECIPIENT_AGREEMENT" data-placeholder="Несколько получателей..." class="chosen-select col-lg-12" multiple tabindex="4" accesskey="" <?php if($trip_id == '3'){$required_state = 'required=""';}else{$required_state = 'hidden="" style="display: none;" disabled=""';} echo $required_state; ?>>
                                <option></option>
                                <?php
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Получатель:</label>
                        <div class="col-sm-10">
                            <select name="RECIPIENT[]" id="RECIPIENT" data-placeholder="Несколько получателей..." class="chosen-select col-lg-12" multiple tabindex="4"required="">
                                ...
                                ?>
                                    <option value="<?php echo $v['EMAIL']; ?>" <?php if($v['EMAIL'] == '' || $v['ID'] == '14' || $v['ID'] == '0'){echo 'disabled';} ?>><?php echo $v['NAME']; ?></option>
                                <?php
                                    }
                                ?>
                            </select>
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Регистрационный номер:</label>
                        <div class="col-sm-10"><input class="form-control" value="" id="REG_NUM" name="REG_NUM" required=""/></div>
                    </div>
                ...
                <div class="mail-body">
                    <button onclick="$('#STATE').val('<?php if(isset($_GET['draft_mail_id'])){echo '00';} else{echo '0';}?>');" type="submit" class="btn btn-sm btn-success" data-toggle="tooltip" data-placement="top" title="Сохранить"><i class="fa fa-save"></i> Сохранить</button>
                </div>
...
<script>
    ...
    function go_to_url(id)
    {
        ...
        location.href = 'http://192.168.5.244'+sim_url+'?trip_id='+id+'&DRAFT_DOC_ID='+<?php echo $_GET['DRAFT_DOC_ID']; ?>;
    }
          ...
          zonaDrop.addEventListener("drop", function(e){
            ...
            if(fileCount > 0)
            {
              for (i = 0; i < fileCount; i = i + 1) {
                ...
                zonaDrop.innerHTML+= "<div class='altay4'><img src='" + img_source + "'/></br> Название " + name +'//'+format+", Тип: " + type +", Размер: " + size +" <a onclick='delete_doc(altay4);'>Delete</a></div>";
