        ...
        private function CNCT_ID($cnct)
        {
            ...
            */
            ...
            foreach($this->dan['clients'] as $k=>$v){
                ...
                $sqlst = "select * from DOBR_DOGOVORS_CLIENTS_NAGRUS where cnct_id = $cnct and id_annuit = $id_annuit";   
            ...
            if($qs > 0){
                ...
                if($qs[0]['JOB_SP'] == '15'){$btn_rastorzh = true;} //СРРиСК - Кнопка расторжение договора
            }
        ...
        {            
            ...
            */
            ...
            $sql = "begin
              ...
              '".$this->array['m_date_calc']."',               
              '".$this->array['m_type_str']."', 
              '".$this->array['m_sicid']."', 
              '".$this->array['m_date_begin']."',
              '".$this->array['m_date_end']."',
            ...
            $raschet_table = $this->db->Select("select * from table(voluntary.calc_new(
            ...
            $pay_sum = $this->array['m_pay_sum_p'];
            $srok = $this->array['m_srok'];
            $agent = $this->array['m_rashod_agent'];//str_replace('.', ',', $this->array['m_rashod'] / 100);
            ...
            $date_begin = $this->array['m_date_begin'];
            $date_end = $this->array['m_date_end'];
            ...
            $result['user_dan'] = array(                
                ...
                "risks"=>array(
                    ...
                    "country"=>$country,
                    "country_uslov"=>$country_uslov,
                    "all"=>$this->array['set_risk_ander']
                ),
            ...
            //$sql = "";
//            if($zabolevaniya !== ''){
//                $sql = "select 'Справочник заболеваний' type_name, name from DOBR_SPR_ZAB where num_id in($zabolevaniya)";
//            }            
//            if($professiya !== ''){
...
//            }
            ...
            if(trim($country_uslov) == ''){$country_uslov = 0;}
            
            if(trim($country) == '0'){$country = 0;}
            if(trim($country_uslov) == '0'){$country_uslov = 0;}
            
            $sqlC = '';
            if($country !== 0){
                if($country_uslov !== 0){                                        
                    $sqlC .= "select 
                    'Справочник стран' type_name,    
                    (select naimen from dobr_country where id = $country)||' '||(select name from DOBR_COUNTRY_NAGRUZ where id = $country_uslov) name
                    from dual";
                }
            }
            ...
            $nagruz_table = array();
            if($sql !== ''){
                $sql = "select s.name type_name, n.name from DOBR_DIC_NAGRUZ n, dobr_dic_spr s
                where n.id_spr = s.id and n.id in($sql) ";
                if($sqlC !== ''){
                    $sql .= " union all ".$sqlC;
                }
                $nagruz_table = $this->db->Select($sql);    
                ...
                if($sqlC !== ''){
                    $nagruz_table = $this->db->Select($sqlC);
                }
            }
                        
            $result['test_sql_n'] = $this->db->sql;
        ...
        private function create_rasp($cnct)
        {
            ...
            $sql = "begin payments_test.CreateRaspDobr($cnct); end;";
            if(!$this->db->ExecProc($sql)){
                echo $this->db->message;
                exit;
            }
