<?php
	class NEW_CONTRACT
    {
        ...
        private function index()
        {
            ...
            if(($branch !== '0000')||($branch !== '1701')){
                $sq = substr($branch, 0, 2);
                $sql .= " and a.branchid like '%$sq%'";
            }
                        
            $sql .= " or a.id in (select b.id from agents_branch_other b where b.branchid = '$branch') order by 2";
                        
            $q = $this->db->Select($sql);
            $this->dan['REGION'] = $this->db->Select("select rfbn_id, name from dic_branch where nvl(asko, 0) = 0 and rfbn_id not in('0000') order by 1");
            $this->dan['AGENT'] = $q;
        }
        
        private function gen_contract_num()
        {   
            $date = $this->array['date'];
            $region = $this->array['region'];
            if(isset($this->array['id_agent'])){
            ...
            $sql = "select gen_contract_num(to_date('$date', 'dd.mm.yyyy'), '$region', '0700', 0, '$role', '$id_agent') contract_num from dual";
            $q = $this->db->Select($sql);
            $q[0]['sql'] = $sql;
        ...
        private function gen_zv_num($region)
        {
            global $active_user_dan;
            $role = $active_user_dan['role'];
            $sql = "selct gen_zv_num('$region', '0700', '$role') zn_num from dual";
            $q = $this->db->Select($sql);
            echo json_encode($q[0]);
            exit;
        }
        ...
        private function search_contragent($txt)
        {
            ...
            WHERE 
                ...
                and a.id = C.OKED_ID
                and S.ID_INSUR = c.id
                and upper(c.name)||c.bin like upper('%$txt%')"
            );
            $dan = $q;
        ...
        {       
            ...
            if ($sum_gfot > $max_gfot){
               $result['message'] = ALERTS::ErrorMin('Превышен лимит ГФОТ-а! Расчет не возможен! ("select * from DIC_PP_KOEF where schp_min <= '.$pst.' and schp_max >= '.$pst.'")');
               echo json_encode($result);                
               exit;
            }
            //----------------------------------------------
