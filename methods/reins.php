<?php
	class REINSURANCE
    {
        ...
        private function id_reins($id)
        {
            ...
            if($id == 0){
                $sql2 = "";
                $sql1 .= 'insert into DIC_REINSURANCE(ID, date_create ';            
                foreach($dan as $k=>$v){                                          
                    $sql1 .= ','.$k;
                    $sql2 .= ",'$v'";
                    $i++;
                }
                $sql1 .= ") values(SEQ_DIC_REINSURANCE.nextval, sysdate $sql2)";
            }else{
                $this->db->Execute("insert into DIC_REINSURANCE_ARC select * from DIC_REINSURANCE where id = $id");
                $sql1 .= "update DIC_REINSURANCE set date_create = sysdate ";
                foreach($dan as $k=>$v){
                    //if($i > 0){$sql1 .= ",";}
                    $sql1 .= ", $k = '$v'";
                    $i++;
           ...
           $this->db->Select("Select * from dic_branch");
        ...
        {        
            $sql = "select 0 id, 0 id_ag, null ID_GROUP, null ocenka from dual
            union all
            select * from REITING_ESTIMATION
            ";
            if($id_agenstv !== 0){
