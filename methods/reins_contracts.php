<?php
	class REINS_CONTRACTS
    {
        ...
        private function search_contragents($d)
        {
            if(trim($d) !== '')
            {
                $this->sql_contracts .= " and b.id in(
                    ...
                    and d.id_insur = c.id
                    and upper(c.name) like upper('%".htmlspecialchars($d)."%')
                )";                
            }
        }
        ...
        {
            //$s = htmlspecialchars($dan);            
            $q = $this->db->Select("select id, name from CONTR_AGENTS where upper(name) like upper('%:sp%') and rownum <= 10", array("sp"=>$dan));            
            echo json_encode($q);
            exit;
        }
