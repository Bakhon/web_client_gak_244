...
";            
        ...
        private function calc($d){
            ...
            if($sex == '0'){
             ...
             if($dop_pokr) {
             $sql_dop_pokr = "select * from DOBR_DIC_TARIF_HANNOVER_DOPFIZ where age = $age and TYPE_DOP_POKR = 2";
     ...
     /*       
      ...
      /*      
        ...
        {            
            ...
            BD.CONTRACT_DATE between ADD_MONTHS(sysdate, -1) and sysdate
             order by bd.id ASC
          ";
  ...
  DR.R_NAME NAME, d.*, bl.*, bd.contract_date, bd.contract_num, bd.ID BORDERO_ID from dobr_dogovors d, bordero_dobr_dogovors_list bl, bordero_dobr_dogovors bd, reinsurance r, dic_reinsurance dr where bd.id = bl.id_contracts and D.ID_NUM = r.id_dobr  and r.id_dobr = bl.cnct_id and R.REINSUR_ID = dr.id and bd.typ = 1 and bl.id_contracts = ".$v['ID']; 
              //  $sql = "select * from bordero_dobr_dogovors_list where id_contracts = ".$v['ID'];
                $q = $this->db->Select($sql);
                
          /*      $sql_fakultativ = "select case
    when D.TYPE_STRAHOVATEL = 0 or D.TYPE_STRAHOVATEL is null then client_name(D.ID_STRAHOVATEL)
    else fond_name(D.ID_STRAHOVATEL)
  end STRAH_NAME, 
  (select sum(bl.PAY_SUM) from bordero_dobr_dogovors_list bl) PAY_SUM_LIST,
  (select count(*) from bordero_dobr_dogovors_list l where L.ID_CONTRACTS = bd.id) CNT_ALL,
  (select DR.NAME from DIC_REINSURANCE_VID dr where DR.ID = r.vid) TYPE, 
  DR.R_NAME NAME, d.*, bl.*, bd.contract_date, bd.contract_num from dobr_dogovors d, bordero_dobr_dogovors_list bl, bordero_dobr_dogovors bd, reinsurance r, dic_reinsurance dr where bd.id = bl.id_contracts and D.ID_NUM = r.id_dobr  and r.id_dobr = bl.cnct_id and R.REINSUR_ID = dr.id and bd.typ = 2 and bl.id_contracts = ".$v['ID']; 
               /* echo '<pre>';
                echo $sql_fakultativ;
                echo '</pre>'; */
                
              //  $qs = $this->db->select($sql_fakultativ);
        ...
        private function contr_num($dan)
        {
            ...
            $sql = "select
                    ...
                    and D.ID_NUM not in (select cnct_id from dobr_dogovors_own) order by D.ID_NUM ASC";     
...
and D.ID_NUM not in (select cnct_id from dobr_dogovors_own) order by D.ID_NUM ASC";            
             ...
             //   print_r($contracts);
                ...
                $sql_reinsurance = "INSERT INTO REINSURANCE(ID, ID_DOBR,  OSNS_SUMV,  OSNS_SUMP,  REINSUR_ID, CONTRACT_NUM,  CONTRACT_date,
                          ...
                          '".$v['contract_date'].".', '".$cn['VIPLAT_BEGIN']."', '".$cn['VIPLAT_END']."', ".str_replace(',', '.', $v['reins_proc']).", ".str_replace(',', '.', $v['reins_prem_proc']).", ".$v['gak_proc'].", ".$v['gak_proc'].", ".$v['reins_summa'].", ".$v['reins_prem_summa'].",  ".str_replace(',', '.', $v['gak_summa']).",
                            ".str_replace(',', '.',$v['reins_prem_summa']).", '', '', '', '', '".$v['vid']."', '', ''
                            )";
        ...
        {            
            ...
            $this->html .= '<div class="pull-right">
                                <a href="reins_export?dobr_contract_num='.$id.'&&export=html" target="_blank" class="btn btn-default"><i class="fa fa-2x fa-html5"></i></a>                                
                                <a href="reins_export?dobr_contract_num='.$id.'&&export=pdf" target="_blank" class="btn btn-default"><i class="fa fa-2x fa-file-pdf-o"></i></a>
                                <a href="reins_export?dobr_contract_num='.$id.'&&export=xls" target="_blank" class="btn btn-default"><i class="fa fa-2x fa-file-excel-o"></i></a>
                            </div>';
