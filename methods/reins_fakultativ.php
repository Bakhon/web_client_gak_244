<?php
	...
	class REINS_REPORT extends REINSURANCE
    {
        private $db;
        private $date_begin;
        private $date_end;
        ...
        private function lists($vid = 1)
        {
            ...
            $sql = "
                ...
                nvl(D.SUM_VOZVR, d.pay_sum_p) pay_sum_p, 
                case when d.state = 18 then '-' end||d.pay_sum_v pay_sum_v, 
                ...
                (select min(b.id) from bordero_contracts b, reinsurance r where r.cnct_id = d.id_head and R.CONTRACT_NUM = b.contract_num and b.id_head is null) id_reins_osn,
                (select resident from contr_agents where id = d.id_insur) resident,
                c.bin,
                o.*
            from 
              contracts d,
            ...
            where 
                ...
                and d.state <> 13 
                and c.id = d.id_insur
                and d.cnct_id not in(select cnct_id from REINSURANCE_OWN)
                and d.state in($lst)
                and d.cnct_id in(
                    select cnct_id from plat_to_1c where id_plat is not null and nvl(pay_sum_d, 0) <> 0
                    union all
                    select cnct_id from pnpd_document where rfpm_id in($pay)
                )                
                and d.vid = $vid                 
                $filter";            
            //echo '<pre>'.$sql.'</pre>';
...
/*            
       ...
       (select contract_num from reinsurance where cnct_id = d.id_head) contract_num_osn,
       (select min(b.id)
          from bordero_contracts b, reinsurance r
         where r.cnct_id = d.id_head
           and R.CONTRACT_NUM = b.contract_num
           and b.id_head is null) id_reins_osn,
       (select resident from contr_agents where id = d.id_insur) resident,
       o.*
  from contracts d, osns_calc o
 where o.cnct_id = d.cnct_id
   and (O.REINS_ID = 4 or O.REINS_ID is null)
   and d.state <> 13   
   and SICID_AGENT in(1620, 3566)       
   and d.cnct_id not in (select cnct_id from REINSURANCE_OWN)
   and d.state in (12, 32)
   and d.vid = 2
union all
select d.contract_num,
       d.contract_date,
       d.date_begin,
       d.date_end,
       fond_name(d.id_insur) strahovatel,
 ...
 where o.cnct_id = d.cnct_id   
   and d.state <> 13   
   and d.cnct_id in(83574, 84380, 84518, 84014, 84013)       
   and d.cnct_id not in (select cnct_id from REINSURANCE_OWN)
   and d.state in (12, 32)
   and d.vid = 2   
order by 5";
*/            
        ...
        private function contr_num($dan)
        {
            ...
            select    
                ...
                bordero.koefppcnctmin(d.cnct_id) min_pp 
            from 
                contracts d 
            where                 
                d.cnct_id in($cn)
                and d.state in(12, 32, 18)";            
            /*
            else D.pay_sum_p -
                nvl( 
        ...
        {                        
                    ...
                    foreach($v as $t=>$d){
                        if($t !== 'skidka'){
                            if(($d == '0') and ($q[0]['VID'] !== '2')){                            
                                echo ALERTS::ErrorMin('Выбранные вами данные не могут равняться нулю(0) или пустому значению! Проверьте страхователя - <b>'.$q[0]['C'].'</b>');
                                exit;
                            }
                        }
                    }                                
            ...
            foreach($dan as $k=>$v){
                ...
                $memo .=  "
                select P.ID_TRUNSH, T.PAY_SUM from bordero_contracts bc, bordero_contracts_list bl, transh t, plat_to_1c p, 
                ...
                if(count($tr) > 0){
                    ...
                    $memo .= "
                    begin bordero.PereraschetContract($id_contracts); end;";
                    $this->db->Execute("begin bordero.PereraschetContract($id_contracts); end;");
                }                                
            }
            
            //Сохраняем в таблицу bordero_contracts
            $memo .= "
            select sum(pay_sum) pay_sum, sum(pay_sum_opl) pay_sum_opl, SUM(sum_s_strah) sum_s_strah 
            ...
            '$pay_sum_OPL',             
            0, 
            '".$dsp['reins_id']."', 0, 2, '$SUM_S_STAH', $id_uv)";    
            
            $memo .= $sql_bordero;                                 
            $this->db->Execute($sql_bordero);            
            
            //echo '<textarea>'.$memo.'</textarea>';
            //exit;
        ...
        {            
            ...
            if($filter == ''){
                $filter .= " and B.CONTRACT_DATE between ADD_MONTHS(sysdate, -3) and sysdate";
            }
            ...
            from 
                bordero_contracts b where B.TYP in(2, 3) 
                and (
                    b.id_head not in(
                        select id from bordero_contracts where typ = 1
                        --union all
                        --select id_head id from bordero_contracts where typ = 3 and id_head not in(select id from bordero_contracts where typ = 1)
                    ) 
                    or b.id_head is null        
                )
                $filter                
        ...
        {            
            ...
            where
                ...
                group by D.CNCT_ID, D.CONTRACT_NUM, BC.ID, BC.CONTRACT_NUM, BC.CONTRACT_DATE, D.ID_INSUR, T.NOM, P.ID_TRUNSH, P.PAY_SUM_D, P.DATE_DOHOD, D.BRANCH_ID
            order by BC.CONTRACT_DATE, BC.CONTRACT_NUM, D.ID_INSUR";
            
            $q = $this->db->Select($sql);            
            //echo '<pre>'.$this->db->sql.'</pre>';
            $this->dan = $q;
        }
        ...
        */
        ...
        private function nestade_list()
        {
            ...
            select 
                ...
                d.id_insur,
                reins_name(o.reins_id) reins_name,
                D.PAY_SUM_P,
                d.pay_sum_v,
                region_name(d.branch_id) region,                    
                p.* 
            from 
                contracts d, PLAT_TO_1C p, osns_calc o
            where p.cnct_id = d.cnct_id
            and d.paym_code like '07%'
            and o.cnct_id = d.cnct_id
            and P.PAY_SUM_D = 0
            --and p.id_trunsh = 1
            and d.date_close is null
            and d.cnct_id != 92234 -- было добавлена Абдисамат Б чтобы убрать Qazaq AIR
            and d.cnct_id not in(
            select cnct_id from bordero_contracts_list union all 
            select cnct_id from bordero_own)
            ");
            
            $this->dan['list'] = $q;
            
            $this->dan['list_reins'] = $this->db->Select("select * from dic_reinsurance");
        }
