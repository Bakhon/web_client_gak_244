...
class XMPP {
	...
	function eventMessage($fromJid, $content, $offline = FALSE)
	{
		...
		if($offline) // офлайн сообщение
		{
			...
			$this->sendMessage($fromJid,"Hi, Thanks for your offline message "); // ответное сообщение
		}
		else // онлайн сообщение
		{
			// можно как то обрабаотать входящее сообщение, а затем написать отправителю, что сообщение получено
			$this->sendMessage($fromJid,"Hi, Thanks for your message");  // ответное сообщение
		}
	...
	*/
	function __construct($webi_conf)
	{
		...
		$this->lastid = 0;
		$this->lastSendTime = 0;
		$this->logEnable = false;//$webi_conf['logtxt'];
		...
		$this->logFileName = "loggerxmpp.log";//$webi_conf['log_file_name'];
		...
		$this->resource = "webi";
		$this->status = "WeBi.Ru";
		$this->tls_off=1;//$webi_conf['tls_off'];
	...
	*/
	function sendXML($xml) {
		...
		if($this->logEnable) { $this->logger("Sending XML>>\n".$xml); }
			...
			if($this->auth) {
				...
				for($i=0;$i<$packetCount;$i++) {
					...
					if((!$this->xmlize->valid) && ($i == $packetCount-1) && (substr($xmlarr[$i],0,14) != '<stream:stream')) {
						...
						if($this->logEnable) { $this->logger("Received XML<<\n".$xml); }
	...
	*/
	function parseStream($arr) {
		...
		$arr["stream:stream"]['@']["xmlns:stream"] != "http://etherx.jabber.org/streams") {
			...
			$xml = '';
			if(  isset($arrStreamFeatures["#"]["starttls"]) AND
			($arrStreamFeatures["#"]["starttls"][0]["@"]["xmlns"] == "urn:ietf:params:xml:ns:xmpp-tls")	AND !$this->tls_off) // если сервер поддерживает SSL и если шифрование не выключено принудительно
			{
