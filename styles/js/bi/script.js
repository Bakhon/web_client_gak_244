$(document).ready(function(){
    ...
    $('body').on('click', '#set_filter', function(){
        ...
        $.post(window.location.href, {"set_filter":fs}, function(data){
            console.log(data);
        });
    });
    ...
    */
    ...
    $('body').on('click', '.view_other', function(){
        var id = $(this).attr('id');
        var data = $(this).attr('data'); 
        var ichart = '<i class="fa fa-line-chart"></i> Графики';
        var itable = '<i class="fa fa-table"></i> Таблица';
        
        var view_table = $('.'+id).css('display');
        if(view_table == 'none'){
            $('.'+id).css('display', 'block');
