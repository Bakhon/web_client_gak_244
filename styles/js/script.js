...
$(function() {  
    ...
    $('.nav').each(function(){
    	$(this).children('li').each(function(){
    		...
    		hr = hr.replace('http://192.168.5.244/', '');
    		var p = hr.split('?');
    		var hrt = p[0];            
    		if(hrt == href){
    		    var data1 = $(this).children('a').attr('data1');
                var data2 = $(this).children('a').attr('data2');
...
$('#send_error').click(function(){
    var txt = $('#error_text').val();
    var url = window.location.href;
    $.post(window.location.href, {"set_error":url, "text_error":txt}, function(data){        
        swal({
            type: "success",
            title: "Отлично!",        
            text: data,
...
$('#btn_send_conf').click(function(){
    var s = $('#send_conference_form').serializeArray();
    var inum = 0;
    var url = '';
    $.each(s, function(i, e){
        if(e.name == 'url_conf'){
    ...
    if(inum == 0){
        alert('Вы ни кого не пригласили на конференцию!');
        return false;
    }else{
        if(url !== ''){
            $.post('admin', $('#send_conference_form').serialize(), function(data){
                window.open(url, '_blank');
                $('#btn_close_conf').click();
                console.log(data);
            });
        }
    }    
